-- TinyInspect
-- https://www.curseforge.com/wow/addons/itemlevel-anywhere

local F, C, L = unpack(select(2, ...))
local pi = F:RegisterModule('PartyIlvl')
...
LibEvent:attachEvent('GROUP_ROSTER_UPDATE', function(self)
    if not C.DB.General.PartyIlvl then return end
...
local function GetRoleIcon(unit)
    ...
    if (role == 'TANK') then
        return '|T' .. C.Assets.Textures.RoleTank .. ':14|t'
    elseif (role == 'HEALER') then
        return '|T' .. C.Assets.Textures.RoleHealer .. ':14|t'
    elseif (role == 'DAMAGER') then
        return '|T' .. C.Assets.Textures.RoleDamager .. ':14|t'
    else
        return ''
    end
end
    ...
    for _, v in pairs(members) do
        if (v.done or v.slevel or v.ilevel > 0) then
            DEFAULT_CHAT_FRAME:AddMessage(format(pattern,
                GetRoleIcon(v.unit),
                C.ClassColors[v.class].colorStr .. v.name .. '|r',
                v.spec and '|cffcccccc(|r' .. C.ClassColors[v.class].colorStr .. v.spec .. '|r|cffcccccc)|r' or '',
...
function pi:OnLogin()
    LibEvent:attachTrigger('PARTY_INSPECT_DONE, PARTY_INSPECT_TIMEOUT', function(self, members)
        if C.DB.General.PartyIlvl then
            if C.DB.General.PartyIlvlToSelf then
