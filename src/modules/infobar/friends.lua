...
function INFOBAR:FriendsPanel_Init()
    ...
    F.CreateFS(
        ...
        F:RgbToHex({ 0.9, 0.8, 0.6 }) .. FRIENDS_LIST, nil, outline and 'NONE' or 'THICK',
        { 'TOPLEFT', 15, -10 }
    )
...
local function buttonOnEnter(self)
    ...
    GameTooltip:SetPoint('TOPRIGHT', infoFrame, 'TOPLEFT', -5, 0)
    GameTooltip:ClearLines()

    if self.isBNet then
        GameTooltip:AddLine(L['Battle.NET Friend'], 0.9, 0.8, 0.6)
        GameTooltip:AddLine(' ')
        ...
        if broadcastText and broadcastText ~= '' then
            ...
            GameTooltip:AddLine(
                format(
                    broadcastString,
                    broadcastText,
                    FriendsFrame_GetLastOnline(broadcastTime)
                ),
                0.3, 0.6, 0.8, 1
            )
        end
...
function INFOBAR:FriendsPanel_CreateButton(parent, index)
    ...
    button.name = F.CreateFS(
        button,
        C.Assets.Fonts.Regular, 13, outline or nil,
        'Tag (name)', nil, outline and 'NONE' or 'THICK',
        { 'LEFT', 25, 0 }
    )
...
local function blockOnEnter(self)
    ...
    INFOBAR:FriendsPanel_Update()
    infoFrame.friendCountText:SetText(format(
        '%s: %s/%s', GUILD_ONLINE_LABEL, totalOnline, totalFriends
    ))
end
...
local function blockOnEvent(self, event, arg1)
    ...
    INFOBAR:FriendsPanel_Refresh()
    self.text:SetText(format('%s: ' .. C.MY_CLASS_COLOR .. '%d', FRIENDS, INFOBAR.totalOnline))
